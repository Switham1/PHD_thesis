%Setup up https://www.softwareheritage.org/2020/05/26/citing-software-with-style/ which allows software citing 
%intended compiler: lualatex
\documentclass[11pt,a4paper]{report}
%better url breaking
\usepackage{xurl}

\usepackage{fullpage}
\usepackage{setspace}
\usepackage{parskip}
\usepackage{titlesec}
\usepackage[section]{placeins}
\usepackage{xcolor}
\usepackage{breakcites}
\usepackage{lineno}
\usepackage{hyphenat}
%quotations: \textquote{quoted text}
%\textcquote[42]{Yu2015}[.]{This is the quoted text and a full sentence with period}
\usepackage{csquotes}
%hyphenation of words and forward slash: architecture\fshyp{}building\hyp{}aware
\usepackage{hyphenat}

\PassOptionsToPackage{hyphens}{url}
\usepackage[colorlinks = true,
						linkcolor = blue,
						urlcolor  = blue,
						citecolor = blue,
						anchorcolor = blue]{hyperref}
%make link to figure show all figure
\usepackage{hypcap}
\usepackage{etoolbox}
%import package to import chapters without nested errors as explained on overleaf
\usepackage{import}
\makeatletter{}

\makeatother{}

\renewenvironment{abstract}
	{{\bfseries\noindent{\abstractname}\par\nobreak}\footnotesize}
	{\bigskip}

%\titlespacing{\section}{0pt}{*3}{*1}
%\titlespacing{\subsection}{0pt}{*2}{*0.5}
%\titlespacing{\subsubsection}{0pt}{*1.5}{0pt}


\usepackage{authblk}
%\usepackage{float} %allow strict figure positioning

\usepackage{placeins}% prevent figures being placed in different section
%then redefine subsections to include float barriers
\let\Oldsection\section{}
\renewcommand{\section}{\FloatBarrier\Oldsection}

\let\Oldsubsection\subsection{}
\renewcommand{\subsection}{\FloatBarrier\Oldsubsection}

\let\Oldsubsubsection\subsubsection{}
\renewcommand{\subsubsection}{\FloatBarrier\Oldsubsubsection}

\usepackage{graphicx}
%add relative path of figures
\graphicspath{{figures/}{../figures/}}
\usepackage[space]{grffile}
\usepackage{latexsym}
\usepackage{textcomp}
\usepackage{longtable}
\usepackage{tabulary}
\usepackage{booktabs,array,multirow}
\usepackage{amsfonts,amsmath,amssymb}
% You can conditionalize code for latexml or normal latex using this.
%Allow rotating of figures
\usepackage{rotating}
\providecommand{\tightlist}{\setlength{\itemsep}{0pt}\setlength{\parskip}{0pt}}%

\AtBeginDocument{\DeclareGraphicsExtensions{.pdf,.PDF,.eps,.EPS,.png,.PNG,.tif,.TIF,.jpg,.JPG,.jpeg,.JPEG}}
%characters
%\usepackage[utf8]{inputenc}
%\usepackage[T1]{fontenc}
\usepackage[UKenglish]{babel}
\selectlanguage{british}
\usepackage[
		backend=biber,
		style=nature,maxcitenames=2, 
		backref=true %add backreferences in bibliography
	]{biblatex}

%add backreferences in bibliography inside page brackets
\DefineBibliographyStrings{english}{%
	backrefpage = {page},% originally "cited on page"
	backrefpages = {pages},% originally "cited on pages"
}  
	
\addbibresource{bibliography/biblio.bib}
%\addbibresource{bibliography/main_biber.bib}
%make letter spacing better
\usepackage[activate={true,nocompatibility},final,tracking=true,factor=1100,stretch=10,shrink=10]{microtype}%spacing=true, kerning=true

%allow use of \eg{}
\usepackage[abbreviations,british]{foreign}

%add units
\usepackage{siunitx} %binary-units=true
\sisetup{detect-all}
\DeclareSIUnit\Molar{\textsc{m}}

%chemistry elements/equations
\usepackage[version=4]{mhchem}

%prevent underfull hbox in biblatex bibliography
\usepackage{etoolbox}
\apptocmd{\sloppy}{\hbadness=10000\relax}{}{}

%use maths package
\usepackage{fontspec, expl3, ucharcat, lualatex-math}
\usepackage{unicode-math}
\setmathfont{texgyrepagella-math.otf}
%use subfile package for chapters
\usepackage{subfiles}

% Edit this header.tex file to include frontmatter definitions and global macros

% Add here any LaTeX packages you would like to load in all document blocks
% \usepackage{xspace}

% Add here any LaTeX macros you would like to load in all document blocks
% \def\example{This is an example macro.}
% Supplementary material
\newcommand{\beginsupplement}{%
				\setcounter{table}{0}
				\renewcommand{\thetable}{S\arabic{table}}%
				\setcounter{figure}{0}
				\renewcommand{\thefigure}{S\arabic{figure}}%
		 }
		 
% Keywords command
\providecommand{\keywords}[1]
{
	\small	
	\textbf{\textit{Keywords}} #1
}
% -----


% Add here any export style-specific LaTeX commands. These will only be loaded upon document export. 
% \paperfield{Subject domain of my document}
% \keywords{keyword1, keyword2}
% \corraddress{Author One PhD, Department, Institution, City, State or Province, Postal Code, Country}
% \fundinginfo{Funder One, Funder One Department, Grant/Award Number: 123456.}

\setlength{\emergencystretch}{2pt}

\begin{document}

\title{Elucidating the Arabidopsis nitrogen-responsive gene regulatory network}



\author[1]{Sam Witham}%
\affil[1]{Earlham Institute}%


\vspace{-1em}



\date{\today}


\begingroup{}
\let\center\flushleft{}
\let\endcenter\endflushleft{}
\maketitle{}
\endgroup{}
\begin{abstract}
	Manipulation of plant growth using engineering biology methods will help to increase crop yields to feed the growing human population.
	Plant growth in response to environmental changes is coordinated by suites of genes working combinatorially within complex gene regulatory networks (GRNs) composed of multitudes of connections, including those between transcription factors (TFs) and recognition sequences associated with the genes they regulate.
	Synthetic biology approaches allow the potential to engineer GRNs by manipulation of regulatory sequences.
	This project aims to: (i) Compare promoter architectural differences between different gene expression categories to guide design of synthetic promoters.
	(ii) Determine how changes in promoter sequence affect the affinity of TFs to their target motifs and how downstream gene expression is affected.
	(iii) Engineer genetic feedback into the nitrogen\hyp{}response network using a nitrogen\hyp{}responsive synthetic promoter controlling a synthetic TF which activates or represses a nitrogen\hyp{}response master regulator.
	To date, I have constructed an analytical pipeline to analyse the promoters of constitutive, variable, non\hyp{}specific and tissue\hyp{}specific genes and have confirmed several significant differences in promoter architecture between gene categories.
	I have also tested the co\hyp{}expression of TFs with promoters of interest in a rapid, transient Arabidopsis protoplast assay to quantify expression with and without the addition of potential cognate TFs, confirming that NLP7 activates the \textit{NIR1} promoter.
	%%% link future work more smoothly with previous part of abstract - why is it the next logical step?
	A CRISPR library has been constructed and delivered to add variation to the promoter of four nitrogen response master regulators. Plant lines containing mutations will be analysed, determining how the downstream gene expression is affected and whether mutations disrupt important promoter motifs.
\end{abstract}

%make a black bar where overfull\
\overfullrule=2cm


\doublespacing{}

\par\null{}

\textbf{\textit{Keywords}} transcription;~\textit{cis}-regulatory element;
\textit{cis}-regulatory module; transcription factor binding site;
transcription start site; transcription start region; gene\hyp{}regulatory network.
\newpage{}
\microtypesetup{protrusion=false}
\tableofcontents{}
%\listoffigures
%\listoftables
\newpage{}
\microtypesetup{protrusion=true}
%add progress review chapter

%\subfile{chapters/chapter1}
%\include{chapters/chapter1}
\subfile{chapters/intro_2021_progressreview.tex}
\subfile{chapters/chapter2}
\subfile{chapters/chapter3}
\subfile{chapters/chapter4}
\subfile{chapters/chapter5}
\subfile{chapters/chapter6}
\subfile{chapters/chapter7}
\FloatBarrier{}
%\section*{References}
%\emergencystretch=3em.
\sloppy{}
\printbibliography{}


\end{document}

