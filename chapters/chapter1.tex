...
\begin{figure}[hbt!]
	...
	\end{center}
\end{figure}
The same analysis was run again using only TFBSs that fall within open chromatin.

\begin{figure}[hbt!]
	\begin{center}
		\capstart
		\includegraphics[width=1\columnwidth]{TFBS_coverage/wholeprom_openchrom/TFBS_coverage_box}
		\caption{
			\textbf{Percentage coverage of TFBS falling within open chromatin across different Arabidopsis \textit{cis}\hyp{}regulatory module (CRMs) expression categories.}
			Promoters were extracted 1000 bp upstream of the annotated Araport 11 \autocite{chengAraport11CompleteReannotation2017} TSS or until the nearest gene.
			5UTRs were extended downstream of the TSS to the closest coding region.  Box plots have box boundaries that represent 25th, 50th (median) and 75th percentiles; whiskers are drawn up to the largest or smallest observed point that falls within 1.5 times the interquartile range.
			Open chromatin peaks derived from the intersect of root and shoot peaks derived from negative control (treated with NaOH) ATAC\hyp{}seq data by \textcite{potterCytokininModulatesContextdependent2018}.
			Control genes are identical in plots A and B.
